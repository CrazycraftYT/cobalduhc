...
client.on('message', async message => {
	...
	if (message.content.startsWith(`${prefix}play`)) {
		...
		message.channel.send(':x: Ta commande est __invalide__ !')
	}
});
...
async function execute(message, serverQueue) {
	...
	if (!voiceChannel) return message.channel.send(':x: Vous devez être dans un channel **vocal** pour pouvoir effectuer cette commande !');
	const permissions = voiceChannel.permissionsFor(message.client.user);
	if (!permissions.has('CONNECT') || !permissions.has('SPEAK')) {
		return message.channel.send('J ai pas la permission !');
	}
...
function skip(message, serverQueue) {
	if (!message.member.voiceChannel) return message.channel.send(':x: Vous devez être dans un channel **vocal** pour pouvoir effectuer cette commande !');
	if (!serverQueue) return message.channel.send(':x: Aucune chanson après ! (Dommage !)');
